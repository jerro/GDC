...
Type *TypeVector::semantic(Loc loc, Scope *sc)
{
    ...
    d_uns64 sz = basetype->size();
    if (basetype->ty != Tsarray || (sz != 16  && sz != 32))
    {   error(loc, "base type of __vector must be a 16 or 32 byte static array, not %s", basetype->toChars());
        return terror;
    }
